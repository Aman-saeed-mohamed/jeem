...
if (isset($_POST['action'])) {
    ...
    $v = mysqli_query($conn,
        "SELECT id FROM orders WHERE id='$order_id' AND shop_id='$shop_id' AND status='accepted'");

    if (mysqli_num_rows($v) === 1) {
        if ($action === 'deliver') {
            mysqli_query($conn, "UPDATE orders SET status='delivered' WHERE id='$order_id'");
    ...
    header("Location: manager_deliveries.php");
    exit();
}

// Accepted orders for THIS shop only
$orders = mysqli_query($conn,
    "SELECT orders.*, users.name AS customer_name
     FROM orders
     LEFT JOIN users ON orders.customer_id = users.id
     WHERE orders.shop_id = '$shop_id' AND orders.status = 'accepted'
...
<head>
    ...
    <title>Deliveries - <?php echo htmlspecialchars($shop['name']); ?></title>
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark sticky-top">
    <div class="container">
        ...
        <div class="navbar-nav ms-auto d-flex flex-row gap-3 align-items-center">
            ...
            <a href="manager_deliveries.php" class="nav-link text-white active">Deliveries</a>
...
<div class="container mt-4">
    <h2 class="mb-4">Deliveries (Accepted Orders)</h2>
    ...
    <table class="table table-bordered table-hover align-middle bg-white">
        <thead class="table-dark">
            <tr>
                ...
                <th>Items</th><th>Total</th><th>Deliver</th><th>Cancel</th>
            </tr>
        </thead>
        <tbody>
        ...
        <tr>
            ...
            <td>
                <form method="post" onsubmit="return confirm('Cancel this delivery? Stock will be restored.');">
                    <input type="hidden" name="order_id" value="<?php echo $oid; ?>">
        ...
        <?php endwhile; else: ?>
        <tr><td colspan="7" class="text-center text-muted py-4">No accepted orders awaiting delivery.</td></tr>
        <?php endif; ?>
        </tbody>
    </table>
    </div>
